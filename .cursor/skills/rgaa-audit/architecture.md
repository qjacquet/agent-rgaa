...
```
rgaa-rules.json  →  agent exécute agent_steps via MCP  →  audit-log.jsonl  →  grid.csv
```

**Aucune heuristique Python** ne note C/NC/NA. Seules les entrées `test_result` dans `audit-log.jsonl` comptent.
...
Règles : [`scoring.md`](scoring.md) — `fail > nt > pass > na`.
...
| `scripts/audit/aggregate-grid.py` | `audit-log.jsonl` → `grid.csv` |
