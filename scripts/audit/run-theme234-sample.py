#!/usr/bin/env python3
"""Save theme 2/3/4 CDP collects and batch-log results for one sample."""
...
def main() -> None:
    if len(sys.argv) < 5:
        print("Usage: run-theme234-sample.py <audit_dir> <sample> <url> <collect_json>", file=sys.stderr)
        sys.exit(1)
    ...
    for theme in ("2", "3", "4"):
        key = f"theme{theme}"
        if key in payload:
            save_and_eval(audit_dir, sample, url, theme, payload[key], skip)
