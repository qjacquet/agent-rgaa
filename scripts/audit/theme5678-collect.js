(() => {
  ...
  let text=0,img=0,comp=0,badText=[],badImg=[],badComp=[],badEmpty=[],badAria=[];
  for (const a of [...document.querySelectorAll('a[href],[role=link]')].filter(v)) {
    ...
    if (!imgL&&!c&&!svgL) text++; else if (imgL) img++; else if (c) comp++;
