#!/usr/bin/env python3
"""Process MCP CDP result file -> save value -> log themes 9-13."""
...
def main() -> None:
    if len(sys.argv) < 5:
        print("Usage: process-mcp-cdp-theme913.py <audit_dir> <sample> <url> <mcp_cdp.json>", file=sys.stderr)
        sys.exit(1)
    audit_dir, sample, url, mcp_path = sys.argv[1:5]
    raw = json.loads(Path(mcp_path).read_text(encoding="utf-8"))
    val_path = Path(f"/tmp/val913-{sample}.json")
    val_path.write_text(json.dumps(extract(raw), ensure_ascii=False), encoding="utf-8")
    subprocess.run([sys.executable, str(RUN), audit_dir, sample, url, str(val_path)], check=True)
